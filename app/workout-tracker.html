<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="Jame Carney Web Developer Portfolio Site">
    <meta name="author" content="James Carney">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <link rel="icon" href="../../favicon.ico">

    <title>Lifestyle Strength and Fitness | App</title>

    <!-- Bootstrap core CSS -->
    <link rel="stylesheet" href="bower_components/foundation/css/foundation.css">

    <!-- Custom styles for this template -->
    <link rel="stylesheet" type="text/css" href="slick/slick/slick.css"> 
    <link href="//maxcdn.bootstrapcdn.com/font-awesome/4.2.0/css/font-awesome.min.css" rel="stylesheet">
    <link rel="stylesheet" href="bower_components/animate.css/animate.min.css">
    <link rel="stylesheet" type="text/css" href="assets/css/style.css">

    <!-- Modernizr for older browsers -->
    <script src="bower_components/foundation/js/vendor/modernizr.js"></script>
  </head>
	<body>
		<div class="wrapper">
			<div class="off-canvas-wrap" data-offcanvas>
				<div class="inner-wrap">
					<nav class="tab-bar sticky">
						<section class="left-small">
							<a class="left-off-canvas-toggle menu-icon" href="#"><span></span></a>
						</section>

						<section class="middle tab-bar-section">
							<h1 class="title">Lifestyle Strength and Fitness</h1>
						</section>
					</nav>

					<aside class="left-off-canvas-menu">
						<ul class="off-canvas-list">
							<li><label>Assessments</label></li>
							<li><a href="#bmi">BMI</a></li>
							<li><a href="workout-tracker.html">Weight Tracker</a></li>
							<li><a href="#">Fitness Tracker</a></li>
						</ul>
					</aside>

					<section class="main-section">
						<form>
							<div class="row">
								<div class="large-12 columns workouts">
									<h1>Workouts</h1>
									<h3>Pick your workout</h3>
									<div class="row workout-selection">
									<!-- have a list of pics that have a label under them of what the workout is... ie. leg day, olympic lifts, deadlift, ect -->

									<!-- Make the last child element of 'workout-selection' have the class 'end' in order to make the columns look nice -->
										<div class="small-6 medium-4 large-3 columns">
											<a href="workout.html"><img src="http://placehold.it/200x200" alt="">
											<p class="lead">Leg Workout</p></a>
										</div>
										<div class="small-6 medium-4 large-3 columns">
											<a href="workout.html"><img src="http://placehold.it/200x200" alt="">
											<p class="lead">Arm Workout</p></a>
										</div>
										<div class="small-6 medium-4 large-3 columns">
											<a href="workout.html"><img src="http://placehold.it/200x200" alt="">
											<p class="lead">Chest Workout</p></a>
										</div>
										<div class="small-6 medium-4 large-3 columns">
											<a href="workout.html"><img src="http://placehold.it/200x200" alt="">
											<p class="lead">Back Workout</p></a>
										</div>
										<div class="small-6 medium-4 large-3 columns">
											<a href="workout.html"><img src="http://placehold.it/200x200" alt="">
											<p class="lead">Olympic Lifts</p></a>
										</div>
										<div class="small-6 medium-4 large-3 end columns">
											<a href="workout.html"><img src="http://placehold.it/200x200" alt="">
											<p class="lead">Plyometric Workout</p></a>
										</div>
									</div>
								</div>
							</div>
						</form> 
						<footer>
							<hr>
							<div class="container text-center clearfix">
								<p>
									<a href="http://facebook.com/carnage91" target="_blank"><i class="fa fa-facebook-square fa-2x"></i></a>
									<a href="http://twitter.com/cawney" target="_blank"><i class="fa fa-twitter-square fa-2x"></i></a>
									<a href="http://youtube.com" target="_blank"><i class="fa fa-youtube-square fa-2x"></i></a>
									<a href="mailto:james.carneydev@gmail.com"><i class="fa fa-envelope-square fa-2x"></i></a>
								</p>
								<p>&copy; 2014 | James Carney</p>
							</div>
						</footer>
					</section>
					<!-- /Keep everything in here -->

					<a class="exit-off-canvas"></a>
				</div> <!-- inner wrap -->
			</div>
			<!-- /off-canvas-wrap -->

		</div><!-- /.wrapper -->

		<!-- JavaScript
		================================================== -->
		<!-- Placed at the end of the document so the pages load faster -->
		<script src="bower_components/foundation/js/vendor/jquery.js"></script>
		<script src="bower_components/foundation/js/vendor/fastclick.js"></script>
		<script src="slick/slick/slick.min.js"></script>

		<!-- Core JS for Foundation -->
		<script src="bower_components/foundation/js/foundation.min.js"></script>
		<script>
			$(document).foundation();
		</script>
		<script>
			$(document).ready(function(){
				$('.slider').slick({
					$('.single-item').slick();
				});
			});
		</script>
	</body>
</html>